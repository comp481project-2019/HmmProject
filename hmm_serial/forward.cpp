...
/*
    perform the forward algorithm on HMM parameters
*/
vector<vector<float> > forward(vector<vector<float> > &transition, vector<vector<float> > &emission,  vector<int> &observations, vector<float> &pi, int stop)
{
...
{ 
    ...
    vector<vector<float> > result = forward(transition, emission, observations, pi, 2);
    cout << "forward\n";
